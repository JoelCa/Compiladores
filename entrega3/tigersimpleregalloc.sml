...
struct
  ...
  open tigerassem
	...
	let val precolored = tigerframe.allRegs
	      ...
	      in if n >= 0 andalso n <= tigerconstants.right16
			...
			val temps =
				let val tempList = 
						let fun f (OPER r, tmplist) = List.concat [#dst r, #src r, tmplist]
			...
			fun rewriteInstr (OPER {assem, dst, src, jump}) =
				let val eset = Splayset.empty String.compare
						...
						fun getTempCol T =
							let fun gtc T [] = if Splayset.member(precoloredSet, T) then T else raise Fail("Temporario no encontrado: "^T)
						...
						val (prevMovs, posMovs) =
							let fun mkgetMov T = movaTemp(getFramePos T, getTempCol T, auxTemp)
