  ...
  let fun error(s, p) = raise Fail ("Error -- línea "^Int.toString(p)^": "^s^"\n")
      fun trexp(VarExp v) = trvar(v)
        ...
        | trexp(OpExp({left, oper=NeqOp, right}, nl)) = 
          let val {exp=explCode, ty=tyl} = trexp left
              val {exp=exprCode, ty=tyr} = trexp right
          in
            if tiposIguales tyl tyr andalso not (tyl=TNil andalso tyr=TNil) andalso tyl<>TUnit then 
        ...
        | trexp(RecordExp({fields, typ}, nl)) =
          let val (tr, tn) = case tabBusca(typ, tenv) of
          ...
          in
            {exp = recordExp(rev r'), ty = TRecord (tr, tn)}
          end
        | trexp(SeqExp(s, nl)) =
        ...
        | trexp(AssignExp({var = SimpleVar s, exp = e}, nl)) =
          let val {ty = expType, exp = expCode} = trexp e
              val {ty = varType, exp = varCode} = trvar ((SimpleVar s),nl)
          in
            case tabBusca(s, venv) of
              SOME (IntReadOnly _) => error(printRef s ^ " es de solo lectura",nl)
        ...
        | trexp(AssignExp ({var, exp}, nl)) =
          let val {ty = expType, exp = expCode} = trexp exp
        ...
        | trexp(ArrayExp({typ, size = e1, init = e2}, nl)) =
          let val {ty = typeSize, exp = sizeCode} = trexp e1
      ...
      and trvar(SimpleVar s, nl) =
        ...
        | trvar(FieldVar(v, s), nl) =
          let val {ty = typeVar, exp = varCode} = trvar (v,nl)
          in
        ...
        | trvar(SubscriptVar(v, e), nl) =
          let val {ty = typeExp, exp = expCode} = trexp e
      ...
      and trdec (venv, tenv) (VarDec ({name,escape = esc,typ=NONE,init},pos)) = 
          let val {ty = typeExp, exp = initCode} = transExp (venv, tenv) init
        ...
        | trdec (venv,tenv) (VarDec ({name,escape = esc,typ=SOME s,init},pos)) =
          ...
          in
            if (not(tiposIguales typeExp typeVar)) then
              ...
              let val venv' = tabRInserta(name,Var {ty=typeVar,access=acc,level=getActualLev()},venv)
              in
                (venv',tenv,[varDec(acc,initCode)])
              end
          end
        | trdec (venv,tenv) (FunctionDec fs) =
          let fun checkNames [] = (~1, "")
                | checkNames (( {name=n,...} , nl)::xs) =
                  let val res = List.all (fn ({name=m,...},_) => m <> n) xs
                  in
                    if res then
                      checkNames xs
                    else
              ...
              fun genEnvEntry ({name = s, params = ps, result = NONE, body = exp}, pos) =
                let val fmlPairs = map genTipo ps
                    ...
                    val f = Func {level = level, label = newName, formals = fmls, result = TUnit, extern = false}
                in
                  ((s,f),(fmlPairs,NONE))
                end
                | genEnvEntry ({name = s, params = ps, result = (SOME n), body = exp}, pos) =
                  let val ttipo = (case tabBusca (n,tenv) of
                      ...
                      val newName = if s = "_tigermain" then s else tigertemp.newlabel()^"_"^s
                      val level = newLevel{parent=topLevel(), name = newName, formals = map (#3) fmlPairs}
                      val f = Func {level = level, label = newName, formals = fmls, result = ttipo, extern = false}
                  in
          ...
          let val firstNL = (#2(hd ts))
                ...
                let fun genP [] res = res
                      ...
                      | genP ({name,ty = RecordTy lf} :: t) res =
                        let fun recorre ({typ = NameTy x, ...} :: t) =
                                (case List.find ((op = rs x) o #name) lt  of
                                  SOME _ => recorre t
                                 | _     => x :: recorre t)
                ...
                let fun buscaEnv t =
                    ...
                    fun precs [] env' = env'
                      | precs ({name, ty=RecordTy lf} :: t) env' =
                        let val lf' = List.foldl (fn ({name, typ=NameTy t, ...}, l) => (name, buscaEnv t) :: l
                                                 | (_, l) => l) [] lf
              ...
              fun fijaNONE [] env = env
                ...
                | fijaNONE ((name, TRecord (lf, u)) :: t) env =
                  let fun busNONE ((s, TTipo (t, r), u), l) = 
                          (case !r of
                ...
                | fijaNONE ((name, TTipo (ty, _)) :: t) env =
                  let val (ta,env') = case tabBusca (ty,env) of
              ...
              fun fijatipos batch env =
                ...
                in env''' end
          in
            let val tenv' = fijatipos (map (#1) ts) tenv
            in
              (venv, tenv', [])
            end
          end
